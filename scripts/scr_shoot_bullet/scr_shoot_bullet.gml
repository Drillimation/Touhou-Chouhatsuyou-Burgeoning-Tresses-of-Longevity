...
function scr_shoot_bullet_enemy(_speed, _direction, _sprite, _color, _x, _y, _aimed, _sound, _function = undefined, _function_arguments = undefined) {
	scr_one_channel_sound(_sound)
	view_xview = camera_get_view_x(view_camera[0]);
	view_yview = camera_get_view_y(view_camera[0]);
	...
	with (inst) {
		...
		direction = _direction;
		if _aimed and instance_exists(enemy) {
			direction = point_direction(x, y, enemy.x, enemy.y) + _direction;
		}
		if _function != undefined {
			effect_script = _function
			script_array = _function_arguments
